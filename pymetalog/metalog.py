...
class metalog:
    ...
    def term_lower_bound(self, tlb):
        ...
        if tlb < 2:
            raise ValueError(
                "term_lower_bound parameter should be greater than or equal to 2"
            )
        if tlb > self.term_limit:
            raise ValueError(
                "term_lower_bound parameter must be less than or equal to term_limit parameter"
            )
    ...
    def append_zvector(self, df_x):
        ...
        if self.boundedness == "su":
            df_x["z"] = -np.log(
                np.array((self.bounds[1] - df_x["x"]), dtype=np.float64)
            )
        if self.boundedness == "b":
            df_x["z"] = np.log(
                np.array(
                    ((df_x["x"] - self.bounds[0]) / (self.bounds[1] - df_x["x"])),
                    dtype=np.float64,
                )
            )
